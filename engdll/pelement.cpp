...
ORBIT_ELE orbitals[8] = {
	...
	39.8151  };
   ##########################################################
   ---------new data in MKPELEMNT.cpp for ASTEROIDS----------
   ##########################################################
*/
...
#define xmul(x)  a = op->x;\
      pp->x = ((((LONG_DOUBLE) a[3] * j->jd_cent + (LONG_DOUBLE)a[2] ) \
      * j->jd_cent + (LONG_DOUBLE)a[1] ) * j->jd_cent + (LONG_DOUBLE)a[0] )

#define xmula(x)  a = op->x;\
      pp->x = ((((LONG_DOUBLE) a[3] * j->jd_cent1989 + (LONG_DOUBLE)a[2] ) \
      * j->jd_cent1989 + (LONG_DOUBLE)a[1] ) * j->jd_cent1989 + (LONG_DOUBLE)a[0] )
...
int prepare_planetary_data( JULIAN *j )
{
 ...
 for ( i = 0, pp = plan_ptr; i < 8; ++i, ++pp ) {
     ...
     aa = murem( aa );
     tmp = (LONG_DOUBLE)a[0] + (LONG_DOUBLE)aa +
	    (( ( (LONG_DOUBLE) a[3] * j->jd_cent ) + (LONG_DOUBLE)a[2] ) * j->jd_cent_sq );
...
/*
 for ( ; i < PLAN_COUNT; ++i, ++pp ) {
     ( handle, op, sizeof( ORBIT_ELE ), );
     a = op->mean_long;
     aa = (LONG_DOUBLE) a[1] * j->jd_cent1989;
     aa = murem( aa );
     tmp = (LONG_DOUBLE)a[0] + (LONG_DOUBLE)aa +
	    (( ( (LONG_DOUBLE) a[3] * j->jd_cent1989 ) + (LONG_DOUBLE)a[2] ) * j->jd_cent_sq1989 );
     pp->mean_long = fmodulus( tmp, 360.0 );
     pp->mean_daily = ( a[1] * 9.856263e-3 ) + ( a[2] + a[3] ) / 36525.0;
     // Using xmul() macro to reduce multiple operations. Note the macro
     // takes the member name in the structure as an argument. See macro
